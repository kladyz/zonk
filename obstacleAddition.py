...
# Set hard-coded obstacle locations
obstacle = pygame.image.load('../img/obstacle.jpg').convert()
grid[0][1] = 2
grid[3][2] = 2
grid[4][2] = 2
    ...
    # only handle key releases for simplicity (KEYUP not KEYDOWN)
      ...
      if event.key == pygame.K_LEFT or event.key == ord('a'):
        if playerCoord[1] > 0:
          ...
          playerCoord[1] -= 1
          #
          if grid[playerCoord[0]][playerCoord[1]] == 2:
            print("The way is blocked. Choose a different route.")
            playerCoord[1] += 1
            grid[playerCoord[0]][playerCoord[1]] = 3
          elif grid[playerCoord[0]][playerCoord[1]] >= 10:
            score += 1
        ...
        if playerCoord[1] < 4:
          ...
          playerCoord[1] += 1
          #
          if grid[playerCoord[0]][playerCoord[1]] == 2: # there is an obstacle
            print("The way is blocked. Choose a different route.")
            playerCoord[1] -= 1 # set coordinate back; they can't move
            grid[playerCoord[0]][playerCoord[1]] = 3 # change icon until user chooses new direction
          elif grid[playerCoord[0]][playerCoord[1]] >= 10:
            score += 1
            print("You have learned "+str(score)+" of the Core 4")           
            grid[playerCoord[0]][playerCoord[1]] = 1
      ...
      for column in range(5):
          ...
          if grid[row][column] == 1:  #player location
              screen.blit(player,((margin+width)*column + margin,
                                 (margin+height)*row+margin))
        # add in obstacle locations
          if grid[row][column] == 3: #an obstacle has been hit; change color of player
              screen.blit(obstacle, ((margin+width)*column + margin,
                                     (margin+height)*row+margin))
